...
class KNN(nn.Module):
  ...
  def forward(self, proj_range, unproj_range, proj_argmax, px, py):
    ...
    '''
    # print(f'size of input-proj_range of KNN: {proj_range.shape}') # torch.Size([64, 2048]) 
    #print(f'size of input-unproj_range of KNN: {unproj_range.shape}') # torch.Size([124668])
    #print(f'size of input-proj_argmax of KNN: {proj_argmax.shape}') # torch.Size([64, 2048])
    #print(f'size of input-px of KNN: {px.shape}') # torch.Size([124668])
    #print(f'size of input-py of KNN: {py.shape}') #  torch.Size([64, 2048]) 
    # get device
    ...
    # print(f'size of proj_range: {proj_range.shape}')
    ...
    proj_range_2 = F.pad(proj_range[None, None, ...],(pad,pad,0,0),mode='circular')
    proj_unfold_k_rang = F.unfold(proj_range_2, kernel_size=(self.search, self.search), padding=(pad, 0))
    ...
    print('shape of proj_argmax: ',proj_argmax.shape) # torch.Size([64, 2048])
    proj_argmax_2 = F.pad(proj_argmax[None, None, ...],(pad,pad,0,0),mode='circular')
    proj_unfold_1_argmax = F.unfold(proj_argmax_2.float(),kernel_size=(self.search, self.search),padding=(pad, 0)).long()
    ...
    print(f'size of output of KNN: {knn_argmax_out.shape}') # torch.Size([124668])
