...
class RDBlock(nn.Module):
  def __init__(self, inplanes, outplanes, block_index, layer_num=2, bn_d=0.1):
    ...
    for i in range(self.layer_num):
      ...
      in_dim = sum(dim_list)
      conv = nn.Sequential( nn.Conv2d(in_dim, in_dim, kernel_size=3, stride=1, padding=1,padding_mode='circular', bias=False))
      ...
      in_dim = sum(dim_list)
      self.local_conv = nn.Sequential(nn.Conv2d(in_dim, outplanes, kernel_size=1, stride=1, padding=0,padding_mode='circular', bias=False),
                                      nn.ReLU())
...
class MultiScaleBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.ms1 = nn.Conv2d(in_channels, out_channels, kernel_size=1, padding=0,padding_mode='circular')
        self.ms2 = nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1,padding_mode='circular')
        self.ms3 = nn.Conv2d(in_channels, out_channels, kernel_size=5, padding=2,padding_mode='circular')
        self.ms4 = nn.Conv2d(in_channels, out_channels, kernel_size=7, padding=3,padding_mode='circular')
...
class DoubleConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.double_conv = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=1,padding_mode='circular'),
            nn.BatchNorm2d(out_channels),
            nn.LeakyReLU(),
        ...
        if not dropout:
            ...
            self.down = nn.Sequential(
                nn.Conv2d(in_channels, out_channels,
                          kernel_size=3, stride=stride, dilation=1, padding=1,padding_mode='circular', bias=False),  # decrease shape, increase dim
...
class Backbone(nn.Module):
    def __init__(self, params):
        ...
        self.merge = nn.Sequential(nn.Conv2d(dim * 3, dim, kernel_size=1, padding=0,padding_mode='circular'),
                                   nn.BatchNorm2d(dim),
                                   nn.LeakyReLU())
