...
class User():
  def __init__(self, ARCH, DATA, datadir, logdir, modeldir, device_ids=None, is_subset=False):
    ...
    # get the data
    parserModule = imp.load_source("parserModule", booger.TRAIN_PATH + '/tasks/semantic/dataset/' + self.DATA["name"] + '/parser.py')
    self.parser = parserModule.Parser(root=self.datadir,
                                      train_sequences=self.DATA["split"]["train"],
                                      valid_sequences=self.DATA["split"]["valid"],
                                      test_sequences=self.DATA["split"]["test"],  # None
  ...
  def infer_subset(self, loader, to_orig_fn):
    ...
    with torch.no_grad():
      ...
      for i, (proj_in, proj_mask, proj_labels, _, path_seq, path_name, p_x, p_y, proj_range, unproj_range, _, _, _, _, npoints) in enumerate(loader):
        ...
        print('shape of proj_output: ',proj_output.shape) # torch.Size([1, 20, 64, 2048])
        proj_argmax = proj_output.argmax(dim=1)
        print('shape of proj_argmax: ',proj_argmax.shape) # torch.Size([1, 64, 2048]
        proj_argmax = proj_argmax.squeeze()
        ...
        print(f'pred_np befor to_orig_fn: {pred_np}')
        pred_np = to_orig_fn(pred_np)
        print(f'pred_np after to_orig_fn: {pred_np}')
        # save scan
