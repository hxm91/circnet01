...
class LocallyConnectedXYZLayer(nn.Module):
  def __init__(self, h, w, sigma, nclasses):
    ...
    self.w = w
    self.padw = w//2
    print(f'size of crf-window: h:{self.h},w:{self.w}')
    print(f'padw: {self.padw}, padh: {self.padh}')
  ...
  def forward(self, xyz, softmax, mask):
    ...
    x_2 = F.pad(x,(self.padw, self.padw,0,0), mode='circular')
    window_x = F.unfold(x_2, kernel_size=(self.h, self.w), padding=(self.padh, 0))
    # window_x = F.unfold(x, kernel_size=(self.h, self.w), padding=(self.padh, self.padw))
    center_x = F.unfold(x, kernel_size=(1, 1), padding=(0, 0))
    y_2 = F.pad(y,(self.padw, self.padw,0,0), mode='circular')
    window_y = F.unfold(y_2, kernel_size=(self.h, self.w), padding=(self.padh, 0))
    # window_y = F.unfold(y, kernel_size=(self.h, self.w), padding=(self.padh, self.padw))
    center_y = F.unfold(y, kernel_size=(1, 1), padding=(0, 0))
    z_2 = F.pad(z,(self.padw, self.padw,0,0), mode='circular')
    window_z = F.unfold(z_2, kernel_size=(self.h, self.w),padding=(self.padh, 0))
    # window_z = F.unfold(z, kernel_size=(self.h, self.w),padding=(self.padh, self.padw))
    center_z = F.unfold(z, kernel_size=(1, 1), padding=(0, 0))

    # sq distance to center (center distance is zero)
    unravel_dist2 = (window_x - center_x)**2 + (window_y - center_y)**2 + (window_z - center_z)**2
    ...
    for i in range(self.nclasses):
      ...
      c_softmax_2 = F.pad(c_softmax,(self.padw, self.padw,0,0), mode='circular')
      unravel_softmax = F.unfold(c_softmax_2,kernel_size=(self.h, self.w),padding=(self.padh, 0))
      # unravel_softmax = F.unfold(c_softmax,kernel_size=(self.h, self.w),padding=(self.padh, self.padw))
...
class CRF(nn.Module):
  ...
  def forward(self, input, softmax, mask):
    # use xyz
    xyz = input[:, 1:4] 
    # print(f'size of input of CRF: {xyz.shape}') # ([8, 3, 64, 1024])
    # iteratively
    for iter in range(self.iter):
      ...
      # print(f'size of reweight_softmax: {reweight_softmax.shape}') # torch.Size([8, 20, 64, 1024])
      # lastly, renormalize
      softmax = F.softmax(reweight_softmax.float(), dim=1)
      # softmax = torch.exp(softmax)
    # print(f'size of output of CRF: {softmax.shape}') # torch.Size([8, 20, 64, 1024])
    return softmax