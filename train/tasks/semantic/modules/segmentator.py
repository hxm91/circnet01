...
class Segmentator(nn.Module):
  def __init__(self, ARCH, nclasses, path=None, path_append="", strict=False, log_dir=None):
    ...
    if  'deeplab' in self.ARCH["backbone"]["name"]:
      ...
      self.head = nn.Sequential(nn.Dropout2d(p=ARCH["head"]["dropout"]),
                                  myconv2d(in_channels=self.decoder.get_last_depth(), out_channels=self.nclasses, pad=(1,1,0,0), kernel_size=3, stride=1, padding=(1,0)))
  ...
  def forward(self, x, mask=None):
    ...
    y = F.softmax(y, dim=1) # y is the output of the network.

    if self.CRF:
      assert(mask is not None)
      print('vor CRF', x.shape)
      print('vor CRF', y.shape)
      y = self.CRF(x, y, mask)
      print('nach CRF', x.shape)
      print('nach CRF', y.shape)
      # print('!!!!!!!', y[0].shape)
    # print('!!!!!!!', y)
    # assert y.shape[0] == 1
    # return y[0]
  ...
  def save_checkpoint(self, logdir, suffix=""):
    ...
    torch.save(self.decoder.state_dict(), logdir +"/segmentation_decoder" + suffix)
    torch.save(self.head.state_dict(), logdir +"/segmentation_head" + suffix)
    if self.CRF:
      torch.save(self.CRF.state_dict(), logdir + "/segmentation_CRF" + suffix)
